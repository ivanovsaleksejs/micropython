...
void IRAM_ATTR esp_neopixel_write_compressed(uint8_t pin, uint8_t *packets, uint32_t numPackets, uint8_t timing) {
    uint8_t *p, *end;
    uint16_t len;
    uint32_t t, pix, time0, time1, period, c, startTime, mask, pinMask;

    pinMask   = 1 << pin;
    p         =  packets;
    end       =  p + numPackets;
    len       = (int)p[0] << 8 | (int)p[1];
    pix       = (int)p[4] << 16 | (int)p[3] << 8 | (int)p[2];
    p += 5;
    mask      = 0x800000;
    startTime = 0;
